...
export default function TicketShow({ auth, tickets, flash }) {
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-50 leading-tight">Vendedor</h2>}
        >
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-sm font-semibold text-gray-600">Assunto</th>
                                    <th className="px-6 py-3 text-left text-sm font-semibold text-gray-600">Descrição</th>
                                    <th className="px-6 py-3 text-left text-sm font-semibold text-gray-600">Data de Criação</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {tickets.map((ticket) => (
                                    <tr key={ticket.id} className="hover:bg-gray-50 transition duration-300 ease-in-out">
                                        <td className="px-6 py-2 text-sm text-gray-700">{ticket.id}</td>
                                        <td className="px-6 py-2 text-sm text-gray-700">{ticket.name}</td>
                                        <td className="px-6 py-2 text-sm text-gray-700">{ticket.phone}</td>
                                        <td className="px-6 py-2 text-sm text-gray-700">{ticket.email}</td>
                                        <td className="px-6 py-2 text-sm text-gray-700">{ticket.status}</td>
                                        <td className="px-6 py-2 text-sm text-gray-700">{ticket.subject}</td>
                                        <td className="px-6 py-2 text-sm text-gray-700">{ticket.description}</td>
                                        <DateCreated ticket={ticket} />
                                        <DateUpdated ticket={ticket} />
                                        <td className="px-6 py-2 text-sm text-gray-700">
                                            <ConfirmDelete routeName="tickets.destroy" id={ticket.id} />
                                        </td>
                                        <td>
